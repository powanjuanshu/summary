...
public class BankRouter {
    ...
    @RequestMapping(value = "/bankRouterBySSL", produces = "text/html;charset=UTF-8")
    @ResponseBody
    public String messageRelayBySSL (HttpServletRequest request) {
        log.info("银行路由bankRouterBySSL--进入");
        try {
            String ip = request.getParameter("ip");
            String port = request.getParameter("port");
            String message = request.getParameter("message");
            ...
            log.info("加载SSL协议...............");
            log.info("银行路由bankRouterBySSL--发送给银行的ip地址为:{}", ip);
            log.info("银行路由bankRouterBySSL--发送给银行的port端口号为:{}", port);
            log.info("银行路由bankRouterBySSL--发送给银行的字节数组转换为十六进制字符串的报文为:{}", message);

            byte[] requestData = ConvertUtil.hexStringToBytes(message); // 将字符串转换成字节数组

            byte[] responseData = send( ip, port, requestData );
            String responseDateStr = ConvertUtil.bytesToHexString(responseData);
            log.info("银行路由bankRouterBySSL--从银行返回的字节数组转换为十六进制字符串的报文为:{}", responseDateStr);

            return responseDateStr;
        } catch (Exception e) {
            log.info("银行路由bankRouterBySSL--发生异常:{}", e);
        }
        return "";
    }
